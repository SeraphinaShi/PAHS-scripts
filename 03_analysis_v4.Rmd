...
df <- df %>% 
  ...
  merge(new_var_df, by = c("SbjctD", "Visit"), all.x = T)
```
...
Results:
```{r}
preform_gee_curFuel_1 <- function(Y, adjust_cfd = F) {
  X = "curFuel_1"
  tmp <- df[!is.na(df[,X]),]
...
Results:
```{r}
preform_gee_curFuel_3 <- function(Y, adjust_cfd = F) {
  X = "curFuel"
  tmp <- df[!is.na(df[,X]),]
...
Results:
```{r}
preform_gee_curFuel_5 <- function(Y, adjust_cfd = F) {
  X = "curFuel_detail"
  tmp <- df[!is.na(df[,X]),]
...
# 2.2. Birth (self-reported) fuel type 
## Primary analysis
### GEE (with confounders, Smokeless ref)
The numbers of observations with each current fuel type:
```{r}
...
preform_gee_brthFuel <- function(Y, adjust_cfd = F) {
  ...
  tmp$brthFuel = factor(tmp$brthFuel, levels = c("Smokeles","Smoky","Wood", "Mix", "Outside of XW/FY"))
...
coefs5 <- data.frame(matrix(NA, ncol = 6))
...
rownames(coefs5) = var_name_list$EEAs
...
coefs5$EAAs = factor(EAAs_names, levels=rev(c(EAAs_names[EAAs_names != "DNAmTL"], "DNAmTL")))
...
p5 <- make_forest_plot_lg(coefs5, "Outside of XW/FY")
...
```
```{r}
cat("The estimated average EAA differences of Outside of XW/FY to Smokeless fuel yupes:")
print(coefs5 %>% 
        mutate(coefficient = round(coefficient, 4),
               std = round(std, 4),
               ci_lower = round(ci_lower, 4),
...
Results:
```{r}
preform_gee_brthFuel_1 <- function(Y, adjust_cfd = F) {
  X = "brthFuel_1"
  tmp <- df[!is.na(df[,X]),]
...
Results:
```{r}
preform_gee_childFuel <- function(Y, adjust_cfd = F) {
  X = "childFuel"
  tmp <- df[!is.na(df[,X]),]
...
Results:
```{r}
preform_gee_childFuel_1 <- function(Y, adjust_cfd = F) {
  X = "childFuel"
  tmp <- df[!is.na(df[,X]),]
  
  tmp <- tmp %>% mutate(childFuel = ifelse(childFuel=="Smoky", "Smoky", "Other"))
  
  tmp$childFuel = factor(tmp$childFuel, levels = c("Other", "Smoky"))
...
```{r gee_childFuel_mix_org_other, fig.width=7, fig.height=5}
fueltype_smoky_mean <- data.frame(matrix(NA, ncol = 6))
fueltype_other_mean <- data.frame(matrix(NA, ncol = 6))
...
Results:
```{r}
preform_gee_childFuel_2 <- function(Y, adjust_cfd = F) {
  X = "childFuel"
  tmp <- df[!is.na(df[,X]),]
  
  tmp <- tmp %>% mutate(childFuel = ifelse(childFuel=="Smoky", "Smoky", 
                                           ifelse(childFuel=="Mix", "Mix", "Smokeless_Wood")))
  
  tmp$childFuel = factor(tmp$childFuel, levels = c("Smokeless_Wood", "Smoky", "Mix"))
...
Results:
```{r}
preform_gee_childFuel_3 <- function(Y, adjust_cfd = F) {
  X = "childFuel"
  tmp <- df[!is.na(df[,X]),]
  ...
  tmp$childFuel = factor(tmp$childFuel, levels = c( "Mix", "Smoky", "Smokeless_Wood"))
...
fueltype_smoky_mean_1$exposure_time = "Current Exposure"
fueltype_smoky_mean_2$exposure_time = "Childhood Exposure"
fueltype_smoky_mean_3$exposure_time = "Cumulative Exposure" 
fueltype_smoky_mean_3 <- fueltype_smoky_mean_3[, !is.na(colnames(fueltype_smoky_mean_3))]
coefs <- rbind(fueltype_smoky_mean_1, fueltype_smoky_mean_2, fueltype_smoky_mean_3)

coefs$exposure_time = factor(coefs$exposure_time, levels = c("Current Exposure","Childhood Exposure", "Cumulative Exposure"))
make_forest_plot_lg(coefs, "Average EAA Difference of Smoky Fuel Type Compared to Smokeless/Mix Fuel Type") +
  ylim(-8,8) + 
  facet_wrap(~ exposure_time)


# smoky vs smokeless
p1 <- make_forest_plot_lg(fueltype_smoky_mean_1, "") + 
  ylim(-6,7) +
  labs(y = " ")
ggsave(file=sprintf("%s/fuelType_current.png",plotFolder), p1, width = 4.5, height = 3, dpi = 1200) 

# smoky vs mix
p2 <- make_forest_plot_lg(fueltype_smoky_mean_2, "") + 
  ylim(-6,7) 
ggsave(file=sprintf("%s/fuelType_childhood.png",plotFolder), p2, width = 4.5, height = 3, dpi = 1200) 


# smoky vs mix
p3 <- make_forest_plot_lg(fueltype_smoky_mean_3, "") + 
  ylim(-6,7) +
  labs(y = " ")
ggsave(file=sprintf("%s/fuelType_cumulative.png",plotFolder), p3, width = 4.5, height = 3, dpi = 1200) 
```
