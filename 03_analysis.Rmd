...
### Linear regression 
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(\text{Smokeles}) + \beta_2 * I(\text{Wood_and_or_Plant}) + \beta_3 * county + \beta_4 * BMI + \beta_5 * ses + \beta_6 * edu + \beta_7 * curStove$$
...
The estimations of $\beta_0$, $\beta_1$ and $\beta_2$ with given $Y$ are shown below, which can be interpreted as "the expected change of Y if switching form the smoky fuel type to the given fuel type, while holding other variables constant".
...
```{r linear_relation_curFuel_mix, fig.width=10, fig.height=3}
...
make_forest_plot(fueltype_smoky_mean, "Slope of current (self-reported) fuel type ") + 
...
# Linear regression
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 *X$$
...
## Current fuel type
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(\text{Smokeles}) + \beta_2 * I(\text{Smoky}) + \beta_3 * I(\text{Wood_and_or_Plant})$$
where $Y$ is one of the epigenetic age accelerations and the reference as the gas/electricity fuel type from the control group.
...
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(Smokeles) + \beta_2 * I(Smoky) + \beta_3 * I(Wood) + \beta_4 * I(Mix)$$
where $Y$ is one of the epigenetic age accelerations and the reference as the gas/electricity fuel type from the control group.
...
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(Smoky) + \beta_2 * I(Mix)$$
where $Y$ is one of the epigenetic age accelerations and the reference as the gas/electricity fuel type from the control group.
...
table(df$curStove)
```
In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(\text{Portable_stove}) + \beta_2 * I(\text{Ventilated}) + \beta_3 * I(\text{Firepit_and_unventilated}) + \beta_4 * I(\text{Mix})$$
...
<!-- ## Stove type -->
<!-- In the following section, we performed traditional linear regression with equation $$Y = \beta_0 + \beta_1 * I(HS) + \beta_2 * I(LS) + \beta_3 * I(Mix) + \beta_4 * I(PS)$$ -->
...
In the following section, we performed traditional regression with equation $$Y = \beta_0 + \beta_1 *log(x)$$
where $Y$ is one of the epigenetic age accelerations, and $X$ is one of the ambient exposure measurements or the urinary measurements. 
