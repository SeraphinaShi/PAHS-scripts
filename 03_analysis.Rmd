...
For these `r length(dup_sbjctD)` subjects, using the estimated epigenetic ages from the **first** visits:
```{r linear_reg_coef_5MC_1, fig.height=3, fig.width=4}
lm_coef_ambient_plots[[1]]
```

For these `r length(dup_sbjctD)` subjects, using the estimated epigenetic ages from the **second** visits:
