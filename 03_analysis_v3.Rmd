...
df$curFuel_org <- df$curFuel
df$brthFuel_org <- df$brthFuel
df$cumFuel_org <- df$cumFuel
...
preform_gee_curFuel <- function(Y, adjust_cfd = F) {
  ...
  tmp$curFuel_org = factor(tmp$curFuel_org, levels = c("Smokeles","Smoky","Wood_and_or_Plant"))
  ...
  rownames(coefs)[rownames(coefs) == "curFuel_orgSmoky"] = "Smoky"
  rownames(coefs)[rownames(coefs) == "curFuel_orgWood_and_or_Plant"] = "Wood_and_or_Plant"  
...
fueltype_smoky_mean_1 <- fueltype_smoky_mean

p1_smokeles <- make_forest_plot_lg(fueltype_smoky_mean, "Smokeless (reference)")
p1_smoky <- make_forest_plot_lg(fueltype_smokeless_mean, "Smoky")
...
preform_gee_brthFuel <- function(Y, adjust_cfd = F) {
  ...
  tmp$brthFuel_org = factor(tmp$brthFuel_org, levels = c("Smokeles", "Smoky","Wood", "Mix"))
  ...
  rownames(coefs)[rownames(coefs) == "brthFuel_orgSmoky"] = "Smoky"
  rownames(coefs)[rownames(coefs) == "brthFuel_orgWood"] = "Wood"
  rownames(coefs)[rownames(coefs) == "brthFuel_orgMix"] = "Mix"
...
```{r gee_brthFuel_mix_org, fig.width=7, fig.height=5}
fueltype_smoky_mean <- data.frame(matrix(NA, ncol = 6))
...
# 2.3. Cumulative lifetime (self-reported) fuel type 
...
preform_gee_cumFuel <- function(Y, adjust_cfd = F) {
  ...
  tmp$cumFuel_org = factor(tmp$cumFuel_org, levels = c("Mix",  "Smoky"))
  ...
  rownames(coefs)[rownames(coefs) == "cumFuel_orgSmoky"] = "Smoky"
  coefs <- coefs %>%
    mutate(ci_lower = coefficient - 1.96 * std,
           ci_upper = coefficient + 1.96 * std,
           sig_level = ifelse(p_val <= 0.001, "<= 0.001",
...
fueltype_smoky_mean_3 <- fueltype_smoky_mean

p2 <- make_forest_plot_lg(fueltype_smoky_mean, "Smoky")
p1 <- make_forest_plot_lg(fueltype_mix_mean, "Mix (reference)")
...
p2 <- p2 + labs(title = "Average EAA difference of cumulative Fuel Type Smoky\n compared to Mix Coal",
